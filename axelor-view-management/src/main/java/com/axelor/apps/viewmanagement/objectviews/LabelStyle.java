//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.3.2 
// See <a href="https://javaee.github.io/jaxb-v2/">https://javaee.github.io/jaxb-v2/</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2023.10.31 at 06:37:00 PM ALMT 
//


package com.axelor.apps.viewmanagement.objectviews;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for LabelStyle.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="LabelStyle"&gt;
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string"&gt;
 *     &lt;enumeration value="default"/&gt;
 *     &lt;enumeration value="important"/&gt;
 *     &lt;enumeration value="success"/&gt;
 *     &lt;enumeration value="warning"/&gt;
 *     &lt;enumeration value="inverse"/&gt;
 *     &lt;enumeration value="info"/&gt;
 *   &lt;/restriction&gt;
 * &lt;/simpleType&gt;
 * </pre>
 * 
 */
@XmlType(name = "LabelStyle")
@XmlEnum
public enum LabelStyle {

    @XmlEnumValue("default")
    DEFAULT("default"),
    @XmlEnumValue("important")
    IMPORTANT("important"),
    @XmlEnumValue("success")
    SUCCESS("success"),
    @XmlEnumValue("warning")
    WARNING("warning"),
    @XmlEnumValue("inverse")
    INVERSE("inverse"),
    @XmlEnumValue("info")
    INFO("info");
    private final String value;

    LabelStyle(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static LabelStyle fromValue(String v) {
        for (LabelStyle c: LabelStyle.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
